 ...
 * }} props
 */
export default function SoloTeamRow({ team, onLeave, onJoin, canJoin = true }) {
    return (
                ...
                <div className="divide-y divide-neutral-800 overflow-hidden rounded-lg border border-neutral-800 md:hidden">
                    {team.slots.map((slot) => (
                        <div
                            ...
                            className="bg-[#111111] px-2 py-2.5"
                        >
                            {slot.occupied ? (
                ...
                <div className="shrink-0 xl:pl-2">
                    {team.joined ? (
                        ...
                        <button
                            ...
                            onClick={() => onJoin?.(team)}
                            disabled={
                                !canJoin || team.availableRoles?.length === 0
                            }
                            className="inline-flex min-h-[44px] w-full items-center justify-center rounded-lg bg-yellow-500 px-6 text-sm font-bold text-black transition-colors hover:bg-yellow-400 disabled:cursor-not-allowed disabled:bg-neutral-700 disabled:text-gray-400 xl:w-auto"
                        >
                            {canJoin ? "Join Team" : "Already Registered"}
                        </button>
                    )}
                </div>
            </div>
